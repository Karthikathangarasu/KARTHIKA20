        ...
        cacheCompatible() {
            if ("1" === wooMultiCurrencyParams.enableCacheCompatible) {
                ...
                r.length && r.each(function(r, t) {
                    c.push(e(t).data("wmc_product_id"))
                });
                ...
                let a = e(".woo-multi-currency.wmc-shortcode");
                c.length && (c = [...new Set(c)]);
                let i = [];
                e(".wmc-cache-value").each(function(c, r) {
                    i.push({
                        value: e(r).data("value"),
                        currency: e(r).data("currency")
                    })
                }), i = [...new Set(i.map(JSON.stringify))].map(JSON.parse), e.ajax({
                    url: wooMultiCurrencyParams.ajaxUrl,
                    type: "POST",
                    data: {
                        action: "wmc_get_products_price",
                        pids: c,
                        shortcode: a.data("layout"),
                        flag_size: a.data("flag_size"),
                        wmc_current_url: a.find(".wmc-current-url").val(),
                        exchange: i
                    },
                    success(c) {
                        if (c.success) {
                            let r = c.data.prices || "",
                                ...
                                i = c.data.exchange || "";
                            if (a.replaceWith(c.data.shortcode), "1" !== wooMultiCurrencyParams.switchByJS && e(".wmc-currency a").unbind(), t && (e(".wmc-sidebar .wmc-currency").removeClass("wmc-active"), e(`.wmc-sidebar .wmc-currency[data-currency=${t}]`).addClass("wmc-active"), e(".wmc-price-switcher .wmc-current-currency i").removeClass().addClass("vi-flag-64 flag-" + c.data.current_country), e(`.wmc-price-switcher .wmc-currency[data-currency=${t}]`).hide(), e(`select.wmc-nav option[data-currency=${t}]`).prop("selected", !0)), "undefined" != typeof woo_multi_currency_switcher && woo_multi_currency_switcher.init(), r) {
                            ...
                            if (i)
                                for (let c in i) i[c].currency ? e(`.wmc-cache-value[data-value=${i[c].value}][data-currency=${i[c].currency}]`).replaceWith(i[c].shortcode) : e(`.wmc-cache-value[data-value=${i[c].value}][data-currency='']`).replaceWith(i[c].shortcode)
                        }
                    }
                })
            }
